    ...
    .map(
      (friend) =>
        ...
        <i class="fa-solid fa-heart fa-lg ${
          favoriteFriend.some((item) => item.id === friend.id)
            ? "heartActive"
            : ""
        }" data-id="${friend.id}"></i>
        <div class="intro">
          ...
          <p class="card-detail">
            <span class="info">Gender: ${
              friend.gender
            }</span><span class="info">Age: ${friend.age}</span>  
          </p>
...
// ${favoriteFriend.some((item) => item.id === friend.id) ? "heartActive" : ""}

/////取localStorage的資料/////
function localStorageFriend() {
...
dataPanel.addEventListener("click", (event) => {
  if (event.target.matches(".fa-circle-info")) {
    ...
    if (event.target.classList.contains("heartActive")) {
      ...
      event.target.classList.add("heartActive");
...
function removeFromFavorite(id) {
  ...
  if (!favoriteFriend || !favoriteFriend.length) return;

  //找出所要刪除的friend在 favoriteFriend這個陣列中的index，如果傳入的 id 在收藏清單中不存在則return結束這個函式
  const friendIndex = favoriteFriend.findIndex((item) => item.id === id);
  if (friendIndex === -1) return;

  //刪除該筆朋友資料
  alert(`即將刪除好友!!`);
  favoriteFriend.splice(friendIndex, 1);

  //把更新後的favoriteFriend，透過直接更改"favoriteFriend"這個key所對應的value，把更新後的favoriteFriend的資料存到value裡
  localStorage.setItem("favoriteFriend", JSON.stringify(favoriteFriend));
}
